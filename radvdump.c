...
static void print_ff(unsigned char *msg, int len, struct sockaddr_in6 *addr, int hoplimit, unsigned int if_index, int edefs)
{
	...
	while (orig_len > 0) {
		...
		case ND_OPT_PREFIX_INFORMATION:{
				...
				addrtostr(AF_INET6, &pinfo->nd_opt_pi_prefix, prefix_str, sizeof(prefix_str));
		...
		case ND_OPT_ROUTE_INFORMATION:{
				...
				if (optlen == 8) {
					...
					addrtostr(AF_INET6, &addr, prefix_str, sizeof(prefix_str));
		...
		case ND_OPT_RDNSS_INFORMATION:{
				...
				addrtostr(AF_INET6, &rdnss_info->nd_opt_rdnssi_addr1, prefix_str, sizeof(prefix_str));
				printf(" %s", prefix_str);

				if (rdnss_info->nd_opt_rdnssi_len >= 5) {
					addrtostr(AF_INET6, &rdnss_info->nd_opt_rdnssi_addr2, prefix_str, sizeof(prefix_str));
					printf(" %s", prefix_str);
				}
				if (rdnss_info->nd_opt_rdnssi_len >= 7) {
					addrtostr(AF_INET6, &rdnss_info->nd_opt_rdnssi_addr3, prefix_str, sizeof(prefix_str));
					printf(" %s", prefix_str);
				}
